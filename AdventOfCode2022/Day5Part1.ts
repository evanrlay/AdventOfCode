...
const performMoves = (moveCount, moveFrom, moveTo) => {
    const moddedCount = moveCount > cratePositions[moveFrom].length ? cratePositions[moveFrom].length : moveCount;
    for (let i = 0; i < moddedCount; i++) {
        const crateToMove = cratePositions[moveFrom].pop();
        cratePositions[moveTo].push(crateToMove);
    }
}
